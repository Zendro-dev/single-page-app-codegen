...
import AminoacidsequenceAttributesPage from './components/aminoacidsequenceAttributesPage/AminoacidsequenceAttributesPage'
import AminoacidsequenceAssociationsPage from './components/aminoacidsequenceAssociationsPage/AminoacidsequenceAssociationsPage'
import AminoacidsequenceTabsA from './components/AminoacidsequenceTabsA'
import AminoacidsequenceConfirmationDialog from './components/AminoacidsequenceConfirmationDialog'
...
export default function AminoacidsequenceCreatePanel(props) {
  ...
  function getInitialValueOkStates() {
    ...
    initialValueOkStates.push({ key: 'accession', valueOk:0 });
    initialValueOkStates.push({ key: 'sequence', valueOk:0 });
  ...
  function doSave(event) {
    //add ids
    values.current.addTranscript_counts = transcript_countsIdsToAdd.current;

    /*
      API Request: createItem
    */
    api.aminoacidsequence.createItem(graphqlServerUrl, values.current)
      .then(response => {
        //Check response
        if (
          response.data &&
  ...
  const handleUntransferFromAdd =(associationKey, itemId) => {
    if(associationKey === 'transcript_count') {
    ...
    <Dialog fullScreen open={open} onClose={handleCancel} TransitionComponent={Transition}>
      <AppBar className={classes.appBar}>
        <Toolbar>
          ...
          <Tooltip title={"Save aminoacidsequence"}>
            <Fab 
              color="secondary" 
      ...
      <div className={classes.root}>
        <Grid container justify='center'>
          <Grid item xs={12}>
            ...
            {/* Attributes Page [0] */}
            <AminoacidsequenceAttributesPage
              hidden={tabsValue !== 0}
            ...
            {/* Associations Page [1] */}
            <AminoacidsequenceAssociationsPage
              hidden={tabsValue !== 1}
              transcript_countsIdsToAdd={transcript_countsIdsToAdd.current}
        ...
        {/* Confirmation Dialog */}
        <AminoacidsequenceConfirmationDialog
...
AminoacidsequenceCreatePanel.propTypes = {
