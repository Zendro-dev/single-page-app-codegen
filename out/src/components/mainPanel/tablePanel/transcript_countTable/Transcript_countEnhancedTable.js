...
import Transcript_countEnhancedTableHead from './components/Transcript_countEnhancedTableHead'
import Transcript_countEnhancedTableToolbar from './components/Transcript_countEnhancedTableToolbar'
import Transcript_countCreatePanel from './components/transcript_countCreatePanel/Transcript_countCreatePanel'
import Transcript_countUpdatePanel from './components/transcript_countUpdatePanel/Transcript_countUpdatePanel'
import Transcript_countDetailPanel from './components/transcript_countDetailPanel/Transcript_countDetailPanel'
import Transcript_countDeleteConfirmationDialog from './components/Transcript_countDeleteConfirmationDialog'
import Transcript_countUploadFileDialog from './components/Transcript_countUploadFileDialog'
...
export default function Transcript_countEnhancedTable() {
  ...
  function getData() {
      ...
      */
      api.transcript_count.getCountItems(graphqlServerUrl, search)
        .then(response => {
          if(
            response.data &&
            response.data.data
          ) {
            var newCount = response.data.data.countTranscript_counts;
            ...
            api.transcript_count.getItems(
              ...
              .then(response => {
                if (
                  response.data &&
                  response.data.data &&
                  response.data.data.transcript_counts) {

                  /**
                    * Debug
                    */
                  console.log("@@newCount: ", newCount);
                  console.log("@@newItems: ", response.data.data.transcript_counts);

                  //set new data
                  setIsOnApiRequest(false);
                  setCount(newCount);
                  setItems(response.data.data.transcript_counts);
                  return;
  ...
  function doDelete(event, item) {
    ...
    */
    api.transcript_count.deleteItem(graphqlServerUrl, item.id)
      .then(response => {
        if(
          response.data &&
          response.data.data
  ...
  return (
    <div className={classes.root}>
      <Grid container justify='center'>
        <Grid item xs={12} md={11} lg={10}>
          <Paper className={classes.paper}>
            ...
            {/* Toolbar */}
            <Transcript_countEnhancedTableToolbar
              search={search}
            ...
            <div className={classes.tableWrapper}>
              <Table stickyHeader size='medium'>
                ...
                {/* Table Head */}
                <Transcript_countEnhancedTableHead
                  order={order}
                  orderBy={orderBy}
                  rowCount={count}
      ...
      {(deleteConfirmationDialogOpen) && (
        <Transcript_countDeleteConfirmationDialog
      ...
      {(uploadFileDialogOpen) && (
        <Transcript_countUploadFileDialog
