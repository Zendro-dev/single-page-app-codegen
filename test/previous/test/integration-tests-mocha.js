...
const gql1_port = 3300;
const spa1_port = 8380;
...
before(async function () {
  ...
  await page.goto(`http://localhost:${spa1_port}`);
  ...
  describe('1.3 <individual> table is empty', function() {
    ...
    it(`${n++}. click on: <individual>`, async function() {
      props = {
        ...
        requests: [`http://localhost:${gql1_port}/graphql`],
        responses: [],
        expectedResponses: 2,
      };
      await clickOn(props);
...
describe('2.1 Associations - one(sql) to many(sql) - add associations - create-panel.', function () {
  ...
  it('01. <transcript_count> table is empty', async function () {
    ...
    await Promise.all([
      ...
      apiResponse = page.waitForResponse(`http://localhost:${gql1_port}/graphql`).then((res) => res.json().then((data) => data.data)),
  ...
  it('03. Create <individual> record with <transcript_count> associations.', async function () {
    ...
    await Promise.all([
      ...
      page.waitForResponse((res) => {
        if(res.url()===`http://localhost:${gql1_port}/graphql`){
