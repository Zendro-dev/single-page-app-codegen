...
# Constants
#
DOCKER_POSTGRES_SERVER1=b_gql_postgres1
DOCKER_POSTGRES_SERVER2=b_gql_postgres2
DOCKER_POSTGRES_CONTAINER1=b_postgres1
DOCKER_POSTGRES_CONTAINER2=b_postgres2
DOCKER_GQL_SERVER1=b_gql_science_db_graphql_server1
DOCKER_GQL_SERVER2=b_gql_science_db_graphql_server2
BASE_PATH="./test/previous"
GQL1_PORT="3300"
GQL2_PORT="3430"
SPA1_PORT="8380"
SPA2_PORT="8087"
TEST_MODELS_INSTANCE1="${BASE_PATH}/test/integration_test_models_instance1"
TEST_MODELS_INSTANCE2="${BASE_PATH}/test/integration_test_models_instance2"
GQL_TEST_MODELS_INSTANCE1="${BASE_PATH}/test/integration_test_models_instance1"
GQL_TEST_MODELS_INSTANCE2="${BASE_PATH}/test/integration_test_models_instance2"
TARGET_DIR="${BASE_PATH}/docker/integration_test_run"
GQL_CODEGEN_DIR="${BASE_PATH}/docker/graphql-server-model-codegen"
...
MANPAGE="${BASE_PATH}/man/integration_test_run.man"
T1=180
T2=600
DO_DEFAULT=true
KEEP_RUNNING=false
...
restartContainers() {
  ...
  # Soft down
  docker-compose -f ${BASE_PATH}/docker/docker-compose-test.yml down -v
  # Msg
  ...
  # Up
  docker-compose -f ${BASE_PATH}/docker/docker-compose-test.yml up -d
  # Msg
  echo -e "@@ Containers up ... ${LGREEN}done${NC}"

  # List
  docker-compose -f ${BASE_PATH}/docker/docker-compose-test.yml ps
...
cleanup() {
  ...
  # Hard down
  docker-compose -f ${BASE_PATH}/docker/docker-compose-test.yml down -v --rmi all
...
stopContainers() {
  ...
  # Stop
  docker-compose -f ${BASE_PATH}/docker/docker-compose-test.yml stop
...
removeVolumes() {
  ...
  # Stop
  docker-compose -f ${BASE_PATH}/docker/docker-compose-test.yml stop ${DOCKER_POSTGRES_SERVER1} ${DOCKER_POSTGRES_SERVER2} ${DOCKER_GQL_SERVER1} ${DOCKER_GQL_SERVER2}

  # Remove db containers (checks existence)
  if [ "$(docker ps -a -f "name=^${DOCKER_POSTGRES_CONTAINER1}$" --format "{{.Names}}")" ];
    then docker rm -v -f ${DOCKER_POSTGRES_CONTAINER1};
  fi
  if [ "$(docker ps -a -f "name=^${DOCKER_POSTGRES_CONTAINER2}$" --format "{{.Names}}")" ];
    then docker rm -v -f ${DOCKER_POSTGRES_CONTAINER2};
  fi
...
waitForGql() {
  ...
  waited=0
  until curl "localhost:${GQL1_PORT}/graphql" > /dev/null 2>&1
  do
    if [ $waited == $T1 ]; then
      # Msg: error
  ...
  until curl "localhost:${GQL2_PORT}/graphql" > /dev/null 2>&1
  do
    if [ $waited == $T1 ]; then
      # Msg: error
...
waitForSpa() {
  ...
  # Wait until the Science-DB GraphQL web-server is up and running
  waited=0
  until curl "localhost:${SPA1_PORT}" > /dev/null 2>&1
  do
    if [ $waited == $T2 ]; then
      # Msg: error
  ...
  until curl "localhost:${SPA2_PORT}" > /dev/null 2>&1
  do
    if [ $waited == $T2 ]; then
      # Msg: error
...
waitForAll() {
  ...
  #Init dbs
  # Instance 1
  docker-compose -f ${BASE_PATH}/docker/docker-compose-test.yml exec ${DOCKER_POSTGRES_SERVER1} \
  ...
  # Instance 2
  docker-compose -f ${BASE_PATH}/docker/docker-compose-test.yml exec ${DOCKER_POSTGRES_SERVER2} \
  bash -c "psql -U sciencedb -d sciencedb_development -P pager=off --single-transaction -f /usr/src/app/integration-test.sql"
  # Msg
...
genCode() {
  ...
  if [ $gql1_status -eq 0 ]; then
    ...
    #
    patch $TARGET_DIR_GQL_INSTANCE1"/validations/with_validations.js" "${BASE_PATH}/test/patches/with_validations.js.patch"
    if [ $? -eq 0 ]; then
    ...
    #
    patch $TARGET_DIR_SPA_INSTANCE1"/src/acl_rules.js" "${BASE_PATH}/test/patches/acl_rules.js.patch"
    if [ $? -eq 0 ]; then
...
upContainers() {
  ...
  # Up
  docker-compose -f ${BASE_PATH}/docker/docker-compose-test.yml up -d --no-recreate
  # Msg
  echo -e "@@ Containers up ... ${LGREEN}done${NC}"
...
buildAll() {
  ...
  # Up
  docker-compose -f ${BASE_PATH}/docker/docker-compose-test.yml up -d --build --force-recreate --renew-anon-volumes
  # Msg
...
doTests() {
 # Msg
  ...
  # Do tests
  mocha --trace-warnings ${BASE_PATH}/test/integration-tests-mocha.js
