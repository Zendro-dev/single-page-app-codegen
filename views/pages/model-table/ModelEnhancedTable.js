...
import <%- nameOnPascal -%>EnhancedTableHead from './components/<%- nameCp -%>EnhancedTableHead'
import <%- nameOnPascal -%>EnhancedTableToolbar from './components/<%- nameCp -%>EnhancedTableToolbar'
import <%- nameOnPascal -%>CreatePanel from './components/<%- nameLc -%>-create-panel/<%- nameCp -%>CreatePanel'
import <%- nameOnPascal -%>UpdatePanel from './components/<%- nameLc -%>-update-panel/<%- nameCp -%>UpdatePanel'
import <%- nameOnPascal -%>DetailPanel from './components/<%- nameLc -%>-detail-panel/<%- nameCp -%>DetailPanel'
import <%- nameOnPascal -%>DeleteConfirmationDialog from './components/<%- nameCp -%>DeleteConfirmationDialog'
import <%- nameOnPascal -%>UploadFileDialog from './components/<%- nameCp -%>UploadFileDialog'
<%if(paginationType === 'cursorBased') {-%>
import <%- nameOnPascal -%>CursorPagination from './components/<%- nameCp -%>CursorPagination'
<%}-%>
...
//Plotly
import <%- nameOnPascal -%>Plotly from '../../../../plots/<%- nameCp -%>Plotly';
...
export default function <%- nameOnPascal -%>EnhancedTable(props) {
  ...
  const [orderBy, setOrderBy] = useState('<%- internalId _%>');
...
<%
  ...
  let queryName1 = 'count'+namePlCp;
  let requestName1 = 'api.'+nameLc+'.getCountItems';
  let queryName2 = paginationType === 'limitOffset' ?  namePlLc : namePlLc+'Connection';
  let requestName2 = paginationType === 'limitOffset' ? 'api.'+nameLc+'.getItems' : 'api.'+nameLc+'.getItemsConnection';
-%>
  /**
    * getCount
    * 
...
<%if(paginationType === 'limitOffset') {-%>
          /*
            Check: empty page
          */
          if((<%- queryName1 %> === (pageRef.current * rowsPerPageRef.current)) && (pageRef.current > 0)) {
            isOnApiRequestRef.current = false;
            setIsOnApiRequest(false);
          ...
          /*
            Check: page overflow
          */
          if((pageRef.current * rowsPerPageRef.current) > <%- queryName1 %>) {
            isOnApiRequestRef.current = false;
            setIsOnApiRequest(false);
            setPage(Math.floor(<%- queryName1 %> / rowsPerPageRef.current));
            return;
          }
<%}-%>
      },
...
<%} else if(paginationType === 'cursorBased') {-%>
    ...
    */
    ...
    let cancelableApiReqB = makeCancelable(api.<%- nameLc -%>.getItemsConnection(
      graphqlServerUrl,
      search,
      orderBy,
      order,
    ...
    cancelableApiReqB
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: '<%- name -%>', query: '<%- queryName2 %>', method: '<%- methodName2 %>', request: '<%- requestName2 %>'}];
          newError.path=['<%- namePlCp %>'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
          console.log("Error: ", newError);
...
<%} else if(paginationType === 'cursorBased') {-%>
        ...
        || !Array.isArray(<%- queryName2 %>.edges)
        || typeof <%- queryName2 %>.pageInfo !== 'object' 
        || <%- queryName2 %>.pageInfo === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = '<%- queryName2 %> ' + t('modelPanels.errors.data.e4', ' received, does not have the expected format.');
        ...
        //get items
        let its = <%- queryName2 %>.edges.map(o => o.node);
        let pi = <%- queryName2 %>.pageInfo;
...
<%}-%>
  ...
  }, [graphqlServerUrl, t, dataTrigger, search, orderBy, order, getCount, showMessage, clearRequestGetData<%if(paginationType === 'cursorBased') {-%>, configurePagination<%}-%><%if(paginationType === 'limitOffset') {-%>, rowsPerPage<%}-%>]);
...
<%
   ...
   * names
   */
  let methodName3 = 'doDelete()';
  let queryName3 = 'delete'+nameCp;
  let requestName3 = 'api.'+nameLc+'.deleteItem';
-%>
  /**
    * doDelete
    * 
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          variant.current='error';
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: '<%- name -%>', query: '<%- queryName3 %>', method: '<%- methodName3 %>', request: '<%- requestName3 %>'}];
        ...
        //check: <%- queryName3 %>
        let <%- queryName3 %> = response.data.data.<%- queryName3 %>;
        if(<%- queryName3 %> === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = '<%- queryName3 %> ' + t('modelPanels.errors.data.e5', 'could not be completed.');
          newError.locations=[{model: '<%- name -%>', query: '<%- queryName3 %>', method: '<%- methodName3 %>', request: '<%- requestName3 %>'}];
          newError.path=['<%- namePlCp %>', `<%- internalId _%>:${item.<%- internalId _%>}` ,'delete'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        /**
         * Type of <%- queryName3 %> is not validated. Only not null is
        ...
        if(response.data.errors) {
          ...
          let withDetails=true;
          variant.current='info';
          newError.message = '<%- queryName3 %> ' + t('modelPanels.errors.data.e6', 'completed with errors.');
          newError.locations=[{model: '<%- name -%>', query: '<%- queryName3 %>', method: '<%- methodName3 %>', request: '<%- requestName3 %>'}];
          newError.path=['<%- namePlCp %>', `<%- internalId _%>:${item.<%- internalId _%>}` ,'delete'];
          newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
      ...
      //error
      .catch((err) => { //error: on <%- requestName3 %>
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
...
<%
   ...
   * names
   */
  let methodName4 = 'getCsvTemplate()';
  let queryName4 = 'csvTableTemplate'+nameCp;
  let requestName4 = 'api.'+nameLc+'.tableTemplate';
-%>
  /**
    * getCsvTemplate
    * 
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          variant.current='error';
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: '<%- name -%>', query: '<%- queryName4 %>', method: '<%- methodName4 %>', request: '<%- requestName4 %>'}];
...
<%if(paginationType === 'limitOffset') {-%>
  function setNewItem(oldItem, newItem) {
    if(!newItem) {
      ...
      let oid = oldItem.<%- internalId _%>;
      //find item
      for(let i=0; i<nitems.length; i++) {
        let iid = nitems[i].<%- internalId _%>;
        if(iid === oid) {
          //set new item
          nitems[i] = newItem;
          setItems(nitems);
          ...
          <Grid container justify='center'>
            <Grid item xs={12}>
              <Paper className={classes.paper}>
                ...
                {/* Toolbar */}
                <<%- nameOnPascal -%>EnhancedTableToolbar
                  permissions={permissions}
                  search={search}
                  showToggleButtons={Boolean(<%- nameOnPascal -%>Plotly)}
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='<%- nameOnPascal -%>EnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='<%- nameOnPascal -%>EnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <<%- nameOnPascal -%>EnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='<%- nameOnPascal -%>EnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'<%- nameOnPascal -%>EnhancedTable-row-'+item.<%- internalId _%>}
                                    hover
                                    onClick={event => handleClickOnRow(event, item)}
                                    role="checkbox"
                                    tabIndex={-1}
                                    key={item.<%- internalId _%>}
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'<%- nameOnPascal -%>EnhancedTable-row-iconButton-detail-'+item.<%- internalId _%>}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                      ...
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'<%- nameOnPascal -%>EnhancedTable-row-iconButton-edit-'+item.<%- internalId _%>}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                      ...
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'<%- nameOnPascal -%>EnhancedTable-row-iconButton-delete-'+item.<%- internalId _%>}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                    ...
                                    <TableCell
                                      ...
                                      padding="checkbox"
                                    >
                                      <Tooltip title={ '<%- internalId _%>: ' + item.<%- internalId _%> }>
                                        <Typography variant='body2' color='textSecondary' display='block' noWrap={true}>{item.<%- internalId _%>}</Typography>
                                      </Tooltip>
                                    </TableCell>
...
<%if(paginationType === 'limitOffset') {-%>
                    ...
                    />
<%} else if(paginationType === 'cursorBased') {-%>
                    <<%- nameOnPascal _%>CursorPagination
                      count={count}
                      rowsPerPageOptions={(count <=10) ? [] : (count <=50) ? [5, 10, 25, 50] : [5, 10, 25, 50, 100]}
...
<%- nameOnPascal -%>EnhancedTable.propTypes = {
