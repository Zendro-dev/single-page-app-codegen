...
import <%- nameOnPascal -%>UpdatePanel from '../<%- nameLc -%>-update-panel/<%- nameCp -%>UpdatePanel'
import <%- nameOnPascal -%>DeleteConfirmationDialog from '../<%- nameCp -%>DeleteConfirmationDialog'
...
<%} else {-%>
import <%- sortedAssociatedModels[i].targetModelOnPascal _%> DetailPanelB from '../../../<%- sortedAssociatedModels[i].targetModelLc _%>-table/components/<%- sortedAssociatedModels[i].targetModelLc _%>-detail-panel/<%- sortedAssociatedModels[i].targetModelCp _%>DetailPanel'
<%}}-%>
...
<%}-%>
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged.<%- name _%>[String(itemState.id)]) {
          
        //updated item
        if(lastModelChanged.<%- name _%>[String(itemState.id)].op === "update"&&
            lastModelChanged.<%- name _%>[String(itemState.id)].newItem) {
              //replace item
              setItemState(lastModelChanged.<%- name _%>[String(itemState.id)].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.<%- name _%>[String(itemState.id)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
